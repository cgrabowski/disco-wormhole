...
public abstract class GLAndroidLiveWallpaperService extends WallpaperService {
	final String TAG = "AndroidLiveWallpaperService";
	static boolean DEBUG = true;
	protected static volatile int runningEngines = 0;

	public GLAndroidLiveWallpaperService () {
		super();
	}
	...
	public class AndroidWallpaperEngine extends Engine {
		...
		public AndroidWallpaperEngine () {
			if (GLAndroidLiveWallpaperService.DEBUG) Log.d(GLAndroidLiveWallpaperService.this.TAG, " > MyEngine() " + hashCode());
		}
...
//			}
		...
		public void onCreate (final SurfaceHolder surfaceHolder) {
			runningEngines++;
			if (GLAndroidLiveWallpaperService.DEBUG) Log.d(TAG, " > onCreate() " + hashCode() + ", running: " + runningEngines);
			super.onCreate(surfaceHolder);
			this.app = new AndroidLiveWallpaper(GLAndroidLiveWallpaperService.this, this);
		...
		public void onDestroy () {
			runningEngines--;
			if (GLAndroidLiveWallpaperService.DEBUG) Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onDestroy() " + hashCode() + ", running: " + runningEngines);
			view.onDestroy();
			if (listener != null)
				listener.dispose();
			app.onDestroy();
		...
		public void onPause () {
			if (GLAndroidLiveWallpaperService.DEBUG) Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onPause() " + hashCode() + ", running: " + runningEngines);
			app.onPause();
			view.onPause();
		}

		public void onResume () {
			if (GLAndroidLiveWallpaperService.DEBUG) Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onResume() " + hashCode() + ", running: " + runningEngines);
			app.onResume();
			view.onResume();
		}
		...
		public void onSurfaceChanged (final SurfaceHolder holder, final int format, final int width, final int height) {
			if (GLAndroidLiveWallpaperService.DEBUG)
				Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onSurfaceChanged() " + isPreview() + " " + hashCode() + ", running: " + runningEngines);
			super.onSurfaceChanged(holder, format, width, height);
		}

		@Override
		public void onSurfaceCreated (final SurfaceHolder holder) {
			if (GLAndroidLiveWallpaperService.DEBUG) Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onSurfaceCreated() " + hashCode() + ", running: " + runningEngines);
			super.onSurfaceCreated(holder);
		}

		@Override
		public void onSurfaceDestroyed (final SurfaceHolder holder) {
			if (GLAndroidLiveWallpaperService.DEBUG) Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onSurfaceDestroyed() " + hashCode()  + ", running: " + runningEngines);
		...
		public void onVisibilityChanged (final boolean visible) {
			if (GLAndroidLiveWallpaperService.DEBUG)
				Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onVisibilityChanged(" + visible + ") " + hashCode());
			if (visible) {
				onResume();
			} else {
				onPause();
		...
		public void onOffsetsChanged (final float xOffset, final float yOffset, final float xOffsetStep, final float yOffsetStep, final int xPixelOffset,
			...
			if (GLAndroidLiveWallpaperService.DEBUG)
				Log.d(GLAndroidLiveWallpaperService.this.TAG, " > onOffsetChanged(" + xOffset + " " + yOffset + " " + xOffsetStep + " "
					+ yOffsetStep + " " + xPixelOffset + " " + yPixelOffset + ") " + hashCode());

			app.postRunnable(new Runnable() {
				@Override
				public void run () {
					GLAndroidLiveWallpaperService.this.offsetChange(listener, xOffset, yOffset, xOffsetStep, yOffsetStep, xPixelOffset, yPixelOffset);	
				}
			});
